...
import { useAutocompletePopupQueries } from './useComposerBoxPopupQueries';
...
export const useAutocompletePopup = <T extends { _id: string; sort?: number }>(
	...
	const commandsRef: AutocompletePopupImperativeCommands<T> = useRef(undefined);
	...
	const setOptionByInput = useStableCallback((): AutocompletePopupOption<T> | undefined => {
		const value = editor?.textBeforeCaret();
